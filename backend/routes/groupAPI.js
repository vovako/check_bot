...
api.post('/',express.json(),async(req,res)=>{
    ...
    for(let i=0;i<groups?.length;i++){
        ...
        if(req?.body?.date){
            ...
            console.log(date)
            if(date)groups[i].ismarked=(await query("select exists(select id_group from Record where day(date_rec)=day(?) and id_group=?) as marked",[req?.body?.date,groups[i].id_group]))[0].marked
        }
    }
